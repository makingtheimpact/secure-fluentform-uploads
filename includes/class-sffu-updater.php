...
function sffu_check_for_updates($transient) {
    ...
    try {
        ...
        if (isset($update_data['up_to_date']) && $update_data['up_to_date'] === true) {
            set_transient('sffu_update_check', 'no_update', SFFU_UPDATE_CACHE_TIME);
            return $transient;
        }
        ...
        if (!empty($update_data['new_version']) && !empty($update_data['package'])) {
            if (version_compare($update_data['new_version'], $current_version, '>')) {
                ...
                set_transient('sffu_update_check', $update_object, SFFU_UPDATE_CACHE_TIME);
...
add_filter('site_transient_update_plugins', 'sffu_check_for_updates');
...
function sffu_clear_update_cache() {
    delete_transient('sffu_update_check');
}

add_action('upgrader_process_complete', 'sffu_clear_update_cache', 10, 0);
add_action('deleted_plugin', 'sffu_clear_update_cache');
add_action('activated_plugin', 'sffu_clear_update_cache');
add_action('deactivated_plugin', 'sffu_clear_update_cache'); 